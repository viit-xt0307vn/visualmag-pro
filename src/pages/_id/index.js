    ...
    async asyncData(context) {
        ...
        if (
            ...
            if (pageData.postDetail.body) {
                pageData.postBodyArray = pageData.postDetail.body
                    .match(/<p>(.*?)<\/p>/g)
                    .map((val) => {
  ...
  -------------------------------------------------- */
    computed: {
        ...
        beforePostAds() {
            ...
            const ads = this.adsPostDetail.find(
                (e) => e.position.id === TYPES.BEFORE_POST
            );
            return ads ? ads : null;
        },
        ...
        beforeContentAds() {
            if (!this.adsPostDetail.length) return null;
            const ads = this.adsPostDetail.find(
                (e) => e.position.id === TYPES.BEFORE_CONTENT
            );
            return ads ? ads : null;
        },
        ...
        beforeParagraphAds() {
            if (!this.adsPostDetail.length) return null;
            const ads = this.adsPostDetail.find(
                (e) => e.position.id === TYPES.BEFORE_PARAGRAPH
            );
            return ads ? ads : null;
        },
        ...
        afterPostAds() {
            if (!this.adsPostDetail.length) return null;
            const ads = this.adsPostDetail.find(
                (e) => e.position.id === TYPES.AFTER_POST
            );
            return ads ? ads : null;
        },
        ...
        afterContentAds() {
            if (!this.adsPostDetail.length) return null;
            const ads = this.adsPostDetail.find(
                (e) => e.position.id === TYPES.AFTER_CONTENT
            );
            return ads ? ads : null;
        },
        ...
        afterParagraphAds() {
            if (!this.adsPostDetail.length) return null;
            const ads = this.adsPostDetail.find(
                (e) => e.position.id === TYPES.AFTER_PARAGRAPH
            );
            return ads ? ads : null;
        },
    },
